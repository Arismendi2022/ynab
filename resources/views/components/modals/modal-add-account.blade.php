...
@push('scripts')
  <script>
    ...
    // 9. Este enfoque asegura que el mensaje de error se elimine
    document.addEventListener('DOMContentLoaded', function () {
      // Selecciona el campo de entrada específico
      const balanceInput = document.querySelector('.balance-input');
    ...
    function saveBudget() {
      ...
      $.ajax({
        ...
        error: function (xhr, status, error) {
          console.error('Error al crear la cuenta:', xhr.responseJSON?.message || error);

          // Asegúrate de que la sección de éxito esté oculta
          sectionSuccess.style.display = 'none';
